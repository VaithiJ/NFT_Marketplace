...
//list all the nfts based on their owners
    const listAllNFT = async () => {
        if (contractLoaded && contract) {
            try {
                const nftIds = await contract.getNFTsByOwner(signerr);
                const nftDataPromises = nftIds.map(async (nftId) => {
                    const nftInfo = {
                        id: nftId[0],
                        owner: nftId[1],
                        creator: nftId[2],
    ...
    return (
        <div>
            <SecondNav />
            <h1 style={{fontSize:"50px", color:"black", marginTop:"100px", fontWeight:"bold"}}>Collection</h1>
            <div className='line1'></div>
            <div className='line2'></div>
            <div style={{marginTop:"40px"}}  className="bg-gradient-to-bl from-blue-50 to-violet-50 flex items-center justify-center lg:h-screen">
                <div className="container mx-auto mx-auto p-4">
                    {nfts.length === 0 ? (
                        <div>
                        <h1>No NFTs to display</h1>
                        <a href='/create'>Mint your First NFT</a>
                        </div>

                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-4">
                            {nfts.map((nft, index) => (
                                <div onClick={() => details(nft.id)} key={index} className="bg-white rounded-lg border p-4">
                                    {nft.image && (
                                        <>
                                            <img
                                                src={nft.image}
